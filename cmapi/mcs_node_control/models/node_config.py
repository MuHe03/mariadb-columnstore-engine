...
from cmapi_server.constants import (
    CMAPI_CONF_PATH, DEFAULT_MCS_CONF_PATH, DEFAULT_SM_CONF_PATH,
    MCS_MODULE_FILE_PATH,
)
from mcs_node_control.models.misc import (
    dequote, read_module_id, get_dbroots_list
)
...
class NodeConfig:
    ...
    # Note: the container.sh script doesn't currently honor the actions
    # generated by this function.
    # Changes here should be reflected in container.sh,
    # until we rewrite how services are managed in the container environment.
    def apply_config(
        ...
        Instance iterates over the xml nodes, yeilding dicts that describes
        system operations to execute to apply the config. It yields dicts each
        describes the services action to be done after MCS applies the config,
        e.g. restart ExeMgr.
        ...
        try:
            ...
            for dbroot in self.get_all_dbroots(new_root):
                try:
                    ...
                    dispatcher_name = dequote(
                        config.get(
                            'Dispatcher', option='name', fallback='systemd'
                        )
                    )
            ...
            # Restart the node
            for msg in [
                {'operation': 'stop', 'service': 'mcs-dmlproc'},
            ...
            new_root = self.get_current_config_root(config_filename)
            if not self.in_active_nodes(new_root):
                module_logger.info(
                    'This node is not in the current ActiveNodes section. '
                    'Not starting Columnstore.'
                )
                return
            else:
            ...
            # TODO: there are no service with such name, only mcs-workernode@1
            yield {'operation': 'start', 'service': 'mcs-workernode'}

            if self.is_primary_node(new_root):
                yield {'operation': 'start', 'service': 'mcs-controllernode'}
            ...
            if self.is_primary_node(new_root):
                yield {'operation': 'start', 'service': 'mcs-dmlproc'}
                yield {'operation': 'start', 'service': 'mcs-ddlproc'}
