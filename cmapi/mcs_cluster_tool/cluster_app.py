...
@app.command()
@handle_output
def stop(timeout:int = typer.Option(0), force:bool = typer.Option(False)):
    """Stop the Columnstore cluster."""
    return ClusterHandler.shutdown(timeout, force, logger=logger)
...
@app.command()
@handle_output
def restart(timeout:int = typer.Option(0), force:bool = typer.Option(False)):
    """Restart the Columnstore cluster."""
    stop_result = ClusterHandler.shutdown(timeout, force, logger=logger)
    if 'error' in stop_result:
        return stop_result
    result = ClusterHandler.start()
