...
def add_node(
    ...
    output_config_filename=None, rebalance_dbroots=True, **kwargs
):
    ...
    try:
        ...
        logging.error(
            'add_node(): Caught exception, config file is unchanged',
            exc_info=True
        )
        raise
    else:
...
def remove_node(
    ...
    try:
        ...
        logging.error(f"remove_node(): Caught exception: '{str(e)}', did not modify the config file")
        raise
    else:
        if output_config_filename is None:
            node_config.write_config(c_root)
...
def rebalance_dbroots(input_config_filename = None, output_config_filename = None):
    ...
    try:
        ...
        logging.error(f"rebalance_dbroots(): Caught exception: '{str(e)}', did not modify the config file")
        raise
    else:
        if output_config_filename is None:
            node_config.write_config(c_root)
