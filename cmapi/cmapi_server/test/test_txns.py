    ...
    def test_broadcast_new_config(self):
        ...
        with start_server():
            try:
                ...
                self.assertEqual(len(result[2]), 1)
                self.assertEqual(result[2][0], myaddr)
                success = helpers.broadcast_new_config(mcs_config_filename, \
                    cmapi_config_filename = cmapi_config_filename, test_mode = True, nodes = result[2])
                helpers.commit_transaction(result[1], cmapi_config_filename, mcs_config_filename) # not specifying nodes here to exercise the nodes = None path
