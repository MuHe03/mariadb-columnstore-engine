...
class MCSProcessManager:
    ...
    def start_node(cls, is_primary: bool, use_sudo: bool = True):
        ...
        """
        # could be incorrect on a first node add, if processes not stopped.
        if cls.is_node_processes_ok(is_primary=is_primary, node_stopped=False):
            logging.info(
                'Seems that all needed MCS processes are alredy started.'
                'Skip starting processes.'
            )
            return
        for prog_name, prog_info in cls._get_sorted_progs(is_primary).items():
            if prog_info.skip_on_startup:
                logging.info(f'Skip starting {prog_name} due to startup rules')
                continue
    ...
    def stop_node(
        ...
        """
        if cls.is_node_processes_ok(is_primary=is_primary, node_stopped=True):
            logging.info(
                'Seems that all MCS processes are alredy stopped.'
                'Skip stopping processes.'
            )
            return
        for prog_name in cls._get_sorted_progs(is_primary, reverse=True):
