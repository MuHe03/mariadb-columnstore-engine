...
class FailoverBackgroundThread(plugins.SimplePlugin):
    ...
    def __init__(self, bus):
        super(FailoverBackgroundThread, self).__init__(bus)
...
if __name__ == '__main__':
    ...
    # TODO: replace config path with variable once that change is merged
    ...
    if turn_on_failover:
        fo = FailoverBackgroundThread(cherrypy.engine)
        fo.subscribe()
        fo.start()
        logging.info('Failover is turned ON by default.')
    else:
    ...
    try:
        ...
        logging.warning(
            ...
            'May be controllernode process isn\'t started.\n'
            'This is just a notification, not a problem.\n'
            'Next detection will started at first node\cluster status check.\n'
            f'This can cause extra {SOCK_TIMEOUT} seconds delay while first \n'
