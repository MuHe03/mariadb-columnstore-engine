    ...
    for node in nodes:
        ...
        for retry in range(5):
            try:
                ...
                logging.warning(f"rollback_txn_attempt(): timeout on node {node}")
            except Exception as e:
                logging.warning(f"rollback_txn_attempt(): got error during request to {node}: {str(e)}")
            else:
                break
            time.sleep(1)
...
def update_revision_and_manager(input_config_filename = None, output_config_filename = None, **kwargs):
    nc = NodeConfig()

    if input_config_filename is None:
        root = nc.get_current_config_root()
    else:
        root = nc.get_current_config_root(config_filename = input_config_filename)

    try:
        rev_node = root.find("./ConfigRevision")
        cur_revision = int(rev_node.text) + 1
        rev_node.text = str(cur_revision)
        root.find("./ClusterManager").text = str(nc.get_module_net_address(root=root, module_id=1))
    except Exception as e:
        logging.error(f"update_revision_and_manager(): caught exception: '{str(e)}', will not write new config")
    else:
