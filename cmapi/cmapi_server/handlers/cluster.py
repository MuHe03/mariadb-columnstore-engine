...
class ClusterHandler():
    ...
    def status(
        ...
        logger.debug('Succesfully finished getting cluster status.')
        return response
    ...
    def start(
        ...
        :raises CMAPIBasicError: if transaction start isn't succesfull
        :raises CMAPIBasicError: if no nodes in the cluster
        :raises CMAPIBasicError: on exception while distributing new config
        :raises CMAPIBasicError: on unsuccesfull distibuting config file
        ...
        if not suceeded:
            rollback_transaction(transaction_id, cs_config_filename=config)
            raise CMAPIBasicError('Starting transaction isn\'t succesfull.')
        ...
        if not broadcast_successful:
            rollback_transaction(transaction_id, cs_config_filename=config)
            raise CMAPIBasicError('Config distribution isn\'t succesfull.')
    ...
    @staticmethod
    def shutdown(
        timeout: int, force: bool, config: str = DEFAULT_MCS_CONF_PATH,
        logger: logging.Logger = logging.getLogger('cmapi_server')
    ) -> dict:
        """Method to stop the MCS Cluster.

        :param timeout: timeout in seconds to wait each cluster node to stop
        :type timeout: int
        :param force: force stopping each node
        :type force: bool
        :param config: columnstore xml config file path,
                       defaults to DEFAULT_MCS_CONF_PATH
        :type config: str, optional
        ...
        for node in active_nodes:
            url = f'https://{node}:8640/cmapi/{get_version()}/node/shutdown'
            payload = {'timeout': timeout, 'force' : force}
            try:
                r = requests.put(
                    url, headers=headers, json=payload, verify=False
                )
                r.raise_for_status()
                response[f'{str(node)}'] = r.json()
            except Exception as err:
                # TODO: Logging error in response,
                #       continue shutting down other nodes
                response[str(node)] = str(err)

        logger.debug('Succesfully finished shutting down the cluster.')
        return response
    ...
    def set_mode(
        ...
        logger.debug(f'Succesfully set cluster mode to {mode}.')
        return response
    ...
    def set_api_key(
        ...
        for node in active_nodes:
            ...
            logger.debug(f'Succesfully set new api key to "{node}".')

        response['timestamp'] = str(datetime.now())
        logger.debug(
            'Succesfully finished setting new API key to all nodes.'
        )
        return response
