...
class SystemdDispatcher(BaseDispatcher):
    ...
    @classmethod
    def _systemctl_call(
        cls, command: str, service: str, use_sudo: bool = True, *args, **kwargs
    ) -> bool:
        ...
        success, _ = cls.exec_command(cmd, *args, **kwargs)
        return success
    ...
    def is_service_running(cls, service: str, use_sudo: bool = True) -> bool:
        ...
        return cls._systemctl_call(
            'is-active --quiet', service, use_sudo, silent=True
        )
    ...
    def start(
        ...
        if is_primary and service == 'mcs-ddlproc':
            ...
            # TODO: check if exist for next releases?
            cls.exec_command(
                'su -s /bin/sh -c "/usr/bin/dbbuilder 7" mysql'
            )

        logging.debug(f'Succesfully started {service_name}.')
    ...
    def stop(
        ...
        if not cls.is_service_running(service_name, use_sudo):
            return True
    ...
    def reload(
        ...
        TODO: Useless? Or we can use it instead of restarting
              services in some cases? Additional error handling?
        """
        service_name = service
        if service_name == 'mcs-workernode':
            service_name = cls._workernode_get_service_name(is_primary)
